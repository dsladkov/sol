...
abstract contract ERC20 is IERC20, IERC20Metadata {
    mapping(address account => uint amount) private _balances; //this account has certain tokens amount

    mapping(address account => mapping(address spender => uint amount))
        private _allowances;

    uint256 private _totalSupply; //Amount of all tokens
    ...
    function _transfer(address from, address to, uint256 _value) internal {
        require(from != address(0) && to != address(0),"Invalid address/ addresses");
